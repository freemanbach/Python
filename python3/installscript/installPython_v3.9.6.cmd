...
set Value="n"
:PROMPT
SET /P Value=Are you sure installing Python-3.9.6 (y/[n])?
IF /I "%Value%" NEQ "y" GOTO END

:: This batch file will show details Windows 10, and install Python v3.9.6.
...
::
::
IF EXIST "python-3.9.5-*.<" (
  echo.Found existing version of Python 3.9.5
  ...
  timeout /T 2 > nul
  echo.No existing version of python 3.8.5 Found.
  echo.
)
::
timeout /T 2 > nul
...
echo.
echo.
C:\Windows\SysWOW64\bitsadmin.exe /transfer PythonDownload /download /priority normal https://www.python.org/ftp/python/3.9.6/python-3.9.6-amd64.exe %userprofile%\Downloads\python-3.9.6-amd64.exe
...
IF EXIST python-3.9.6-amd64.exe (
...
echo.
echo.Installing Python version 3.9.6.
...
del /s %userprofile%\Downloads\python-3.9.6-amd64.exe >nul 2>&1
