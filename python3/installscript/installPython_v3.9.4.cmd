...
set Value="n"
:PROMPT
SET /P Value=Are you sure installing Python39 (y/[n])?
...
::
::
IF EXIST "python-3.9.4-*.<" (
  echo.Found existing version of Python 3.9.4
  ...
  timeout /T 2 > nul
  echo.No existing version of python 3.9.4 Found.
  echo.
)
::
timeout /T 2 > nul
::
IF EXIST "python-3.9.3-*.<" (
  echo.Found existing version of Python 3.9.3
  del python-3.9.3-amd64.exe
  echo.Deleting existing version of Python
  echo.
  timeout /T 2 > nul
) ELSE (
  timeout /T 2 > nul
  echo.No existing version of python 3.9.3 Found.
  echo.
)
::
timeout /T 2 > nul
::
IF EXIST "python-3.9.2-*.<" (
  echo.Found existing version of Python 3.9.2
  del python-3.9.2-amd64.exe
  echo.Deleting existing version of Python
  echo.
  timeout /T 2 > nul
) ELSE (
  timeout /T 2 > nul
  echo.No existing version of python 3.9.2 Found.
  echo.
)
...
timeout /T 2 > nul
::
IF EXIST "python-3.9.0-*.<" (
  echo.Found existing version of Python 3.9.0
  ...
  timeout /T 2 > nul
  echo.No existing version of python 3.9.0 Found.
  echo.
)
...
echo.
echo.
bitsadmin /transfer PythonDownload /download /priority normal https://www.python.org/ftp/python/3.9.4/python-3.9.4-amd64.exe %userprofile%\Downloads\python-3.9.4-amd64.exe
...
IF EXIST python-3.9.4-amd64.exe (
...
echo.
echo.Installing Python version 3.9.4.
...
echo.
:check
if exist C:\Python39\Tools\pynche\Main.py (
...
timeout /T 4 > nul
C:\Python39\scripts\pip.exe install --user requests diceware scipy numpy pandas matplotlib seaborn wordcloud wikipedia pillow pandas_datareader yfinance
timeout /T 4 > nul
...
echo.
::
echo set PATH=C:\Python39;%PATH% > %userprofile%\runme.bat
del /s %userprofile%\Downloads\python-3.9.4-amd64.exe >nul 2>&1
