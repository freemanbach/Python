...
:check_Permissions
    ...
    if %errorLevel% == 0 (
        ...
        echo run this script to install Python v3.9.10 Silently.
        echo Exiting....
        goto end
    )
::    pause >nul
...
set Value="n"
:PROMPT
SET /P Value=Are you sure installing Python-3.9.10 (y/[n])?
IF /I "%Value%" NEQ "y" GOTO END

:: This batch file will show details Windows 10, and install Python v3.9.10
...
echo.
echo.
C:\Windows\SysWOW64\bitsadmin.exe /transfer PythonDownload /download /priority normal https://www.python.org/ftp/python/3.9.10/python-3.9.10-amd64.exe C:\Users\%USERNAME%\Downloads\python-3.9.10-amd64.exe
::
echo.
echo.
timeout /T 2 > nul
...
timeout /T 4 > nul
::
IF EXIST python-3.9.10-amd64.exe (
...
echo.============================
echo.
echo.Installing Python version 3.9.10
...
echo.
:check
if exist C:\Python3910\Tools\pynche\Main.py (
...
C:\Python3910\python.exe -m pip install --upgrade pip
::
:: Quick Pause
timeout /T 2 > nul
echo.
...
:: Quick Pause
timeout /T 1 > nul
C:\Python3910\Scripts\pip.exe install --user wheel
timeout /T 1 > nul
echo done
::
echo.
...
echo.============================
::
echo set PATH=C:\Python3910;%PATH% > C:\Users\%USERNAME%\runme.bat
del /s C:\Users\%USERNAME%\Downloads\python-3.9.10-amd64.exe >nul 2>&1
