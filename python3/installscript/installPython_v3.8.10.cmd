...
set Value="n"
:PROMPT
SET /P Value=Are you sure installing Python-3.8.10 (y/[n])?
IF /I "%Value%" NEQ "y" GOTO END

:: This batch file will show details Windows 10, and install Python v3.8.10.
...
timeout /T 2 > nul
::
IF EXIST "python-3.8.9-*.<" (
  echo.Found existing version of Python 3.8.9
  del python-3.8.9-amd64.exe
  echo.Deleting existing version of Python
  echo.
  timeout /T 2 > nul
) ELSE (
  timeout /T 2 > nul
  echo.No existing version of python 3.8.9 Found.
  echo.
)
::
timeout /T 2 > nul
::
IF EXIST "python-3.8.8-*.<" (
  echo.Found existing version of Python 3.8.8
  del python-3.8.8-amd64.exe
  echo.Deleting existing version of Python
  echo.
  timeout /T 2 > nul
) ELSE (
  timeout /T 2 > nul
  echo.No existing version of python 3.8.8 Found.
  echo.
)
::
echo.
...
echo.
echo.
C:\Windows\SysWOW64\bitsadmin.exe /transfer PythonDownload /download /priority normal https://www.python.org/ftp/python/3.8.10/python-3.8.10-amd64.exe %userprofile%\Downloads\python-3.8.10-amd64.exe
...
IF EXIST python-3.8.10-amd64.exe (
...
echo.
echo.Installing Python version 3.8.10.
...
del /s %userprofile%\Downloads\python-3.8.10-amd64.exe >nul 2>&1
