...
IF EXIST "python-3.8.6-*.<" (
  echo.Found existing version of Python 3.8.6 from Download directory
  del python-3.8.6-amd64.exe
  echo.Deleting existing version of Python from Download directory
  echo.
  timeout /T 2 > nul
) ELSE (
  timeout /T 2 > nul
...
IF EXIST "python-3.8.7-*.<" (
  echo.Found existing version of Python 3.8.7 from Download directory
