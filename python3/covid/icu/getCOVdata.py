...
def processJSON(jsdata):
    ...
    for i in data:
        ...
        if str(i['inIcuCumulative']).isdigit():
            death.append(int(i['inIcuCumulative']))
        else:
            death.append(0)
        if str(i['inIcuCurrently']).isdigit():
            deathinc.append(int(i['inIcuCurrently']))
        else:
            deathinc.append(0)
    # reverse data points
    deathinc.reverse()
...
def writeData( a, b, c, nf ):
    ...
    head = "Date,"+ nf.upper() + "DATA.InIcuCum," +  nf.upper() + "DATA.InIcuCurr\n"
