...
def processJSON(jsdata):
    ...
    for i in data:
        ...
        if str(i['totalTestResults']).isdigit():
            death.append(int(i['totalTestResults']))
        else:
            death.append(0)
        if str(i['totalTestResultsIncrease']).isdigit():
            deathinc.append(int(i['totalTestResultsIncrease']))
        else:
            deathinc.append(0)
    # reverse data points
    deathinc.reverse()
...
def writeData( a, b, c, nf ):
    ...
    head = "Date,"+ nf.upper() + "DATA.TotalTestResults," +  nf.upper() + "DATA.TotalTestResultsInc\n"
