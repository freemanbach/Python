...
def processJSON(jsdata):
    ...
    for i in data:
        ...
        if str(i['hospitalized']).isdigit():
            death.append(int(i['hospitalized']))
        else:
            death.append(0)
        if str(i['hospitalizedCumulative']).isdigit():
            deathinc.append(int(i['hospitalizedCumulative']))
        else:
            deathinc.append(0)
    # reverse data points
    deathinc.reverse()
...
def writeData( a, b, c, nf ):
    ...
    head = "Date,"+ nf.upper() + "DATA.Hosp," +  nf.upper() + "DATA.HospCum\n"
