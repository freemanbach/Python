...
def processJSON(jsdata):
    ...
    for i in data:
        ...
        if str(i['onVentilatorCumulative']).isdigit():
            death.append(int(i['onVentilatorCumulative']))
        else:
            death.append(0)
        if str(i['onVentilatorCurrently']).isdigit():
            deathinc.append(int(i['onVentilatorCurrently']))
        else:
            deathinc.append(0)
    # reverse data points
    deathinc.reverse()
...
def writeData( a, b, c, nf ):
    ...
    head = "Date,"+ nf.upper() + "DATA.OnVentCum," +  nf.upper() + "DATA.OnVentCurr\n"
